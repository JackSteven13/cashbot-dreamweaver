...
import { REFERRAL_CODES_TABLE, ReferralCodeInsert } from '@/integrations/supabase/types/referralCodes';
...
const AccessCodeGenerator = () => {
  ...
  const handleGenerateCodes = async () => {
    ...
    try {
      ...
      for (let i = 0; i < codeCount; i++) {
        ...
        // Enregistrer le code en base de données avec le typesafety correct
  ...
  return (
    <Card className="w-full">
                    ...
                    <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                      <thead className="bg-gray-50 dark:bg-gray-800">
                        <tr>
                          ...
                          <th className="px-2 sm:px-3 py-2 text-right text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">
                            Action
                          </th>
                        </tr>
                      </thead>
                      <tbody className="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-800">
                        {generatedCodes.map((item, index) => (
                          <tr key={index} className="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                            <td className="px-2 sm:px-3 py-2 whitespace-nowrap text-xs sm:text-sm font-mono">
                              {item.code}
                            </td>
                            <td className="px-2 sm:px-3 py-2 whitespace-nowrap text-xs sm:text-sm text-gray-500 dark:text-gray-400">
                              {new Date(item.generated_at).toLocaleString()}
                            </td>
                            <td className="px-2 sm:px-3 py-2 whitespace-nowrap text-right text-sm">
                              <Button
                                variant="ghost"
                                size="sm"
      ...
      <CardFooter className="flex flex-col items-start border-t pt-4 sm:pt-6">
